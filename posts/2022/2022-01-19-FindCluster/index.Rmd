...
description: "Masking the cluster of a brain image."
author: Feng Sang
date: 2022-01-19
slug: blog
...
```{r, echo=FALSE, fig.cap='test-binary.nii可视化结果。'}
knitr::include_graphics('./img/fig-1.png')
```


上述代码运行后，对于mask中的每一个cluster会得到一个.nii文件，同时也会得到一个混合了所有cluster的.nii文件。结果如Figure 2所示。

```{r, echo=FALSE, fig.cap='混合了所有cluster的结果，其中每个cluster用一个整数标记。'}
knitr::include_graphics('./img/fig-2.png')
```
